...
import { Image as ImageIcon, Video, RefreshCw, Download, Sparkles, Loader2, Youtube, Instagram, ArrowLeft, ArrowRight, Layers, ExternalLink, Wand2, Building2, ChevronDown, ChevronUp, CheckCircle2 } from 'lucide-react';
...
export default function WizardVisuals({ 
  masterBrief, 
  onNext, 
  onBack,
  attachedFiles = []
  ...
  const [referenceEmailText, setReferenceEmailText] = useState<string>(() => {
    return masterBrief?.overview?.campaignName 
      ? `Campaña: ${masterBrief.overview.campaignName}. Audiencia: ${masterBrief.overview.targetAudience || ''}. Tono: ${masterBrief.overview.tone || ''}`
      : storage.getBriefingContext() || '';
  });
  ...
  const handleRefineWithArchitecturalTemplate = async () => {
    if (!imagePrompt.trim()) {
      setError('Escribe una descripción base antes de refinar con arquitectura.');
      return;
    }
    ...
    try {
      ...
      if (data.success && data.refinedEnglishPrompt) {
        ...
        setError(data.error || 'No se pudo refinar el prompt.');
      }
    } catch (err: any) {
  ...
  const [imagePrompt, setImagePrompt] = useState(() => {
    return masterBrief?.instagram?.imageIdea || masterBrief?.facebook?.imageIdea || 'Fotografía de producto profesional comercial, 8k, iluminación de estudio softbox, acabado de alta gama...';
  });
  ...
  // Collect available images from briefing files and generated history
  const availableReferenceImages = [
    ...attachedFiles.filter(f => f.type === 'image' && f.dataUrl).map(f => ({
      id: f.id,
      name: f.name,
      url: f.dataUrl!
    })),
    ...savedHistory.map(item => ({
      id: item.id,
      name: `Render ${new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`,
      url: item.url
    }))
  ];
    ...
    <div className="space-y-6 max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 animate-in fade-in zoom-in-95 duration-300">
      <div className="text-center space-y-2 mb-6">
        <h2 className="text-2xl font-black text-slate-900 dark:text-white flex items-center justify-center gap-2">
          <ImageIcon className="text-blue-600" /> Paso 3: Generación Visual (100% Gemini Imagen & Veo)
        </h2>
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Convierte tu Briefing en Assets Visuales comerciales con alta fidelidad y consistencia de marca.
        </p>
      </div>

      {/* Interactive Prompt Structure Guide */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-gray-200 dark:border-slate-800 shadow-sm space-y-4">
            <h3 className="font-bold flex items-center gap-2 text-blue-600 dark:text-blue-400 text-sm">
              <ImageIcon size={18} /> Asset Estático (Google Gemini Imagen)
            </h3>
            
            <div className="space-y-2">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300">
                  Prompt Visual de la Campaña:
                </label>
              ...
              <textarea 
                ...
                className="w-full bg-slate-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-3 text-xs text-slate-800 dark:text-slate-200 leading-relaxed focus:border-blue-500 outline-none"
                rows={4}
                placeholder="Describe la escena publicitaria o usa el Asistente IA para fidelidad con tu referencia..."
              />

              {socialSpecs && (
                <div className="p-2.5 bg-blue-50/70 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-900/50 text-xs space-y-0.5">
                  <div className="text-[10px] text-blue-800 dark:text-blue-300 leading-snug">
                    • <strong>Safe-Zone:</strong> {socialSpecs.safeZoneTip || 'Composición centrada para feed y móvil.'}
                  </div>
                </div>
              )}
            </div>

            {/* Visual Reference Picker */}
            <ImageReferencePicker
              referenceImage={referenceImage}
              onSelectReference={(url) => setReferenceImage(url)}
              availableImages={availableReferenceImages}
              label="Referencia Visual de Producto / Marca"
              referenceEmailText={referenceEmailText}
              onUpdateEmailText={setReferenceEmailText}
            />

            {/* Architectural Photography Prompt Refiner Block */}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">
                      Ópticas tilt-shift, iluminación física calculada y micro-texturas táctiles.
                    </p>
                  </div>
                </div>
                <button
            ...
            <div className="space-y-2">
              ...
              <select 
                ...
                className="w-full bg-slate-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-2.5 text-xs font-medium text-slate-700 dark:text-slate-200 outline-none focus:border-blue-500"
              >
                ...
                <option value="Macro Detalle de Texturas y Acabados">Macro Detalle de Texturas y Acabados</option>
              </select>
            </div>

            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Formato / Relación de Aspecto:</label>
                <button
                  type="button"
                  onClick={() => setImageResolution(imageResolution === '2K' ? '1K' : '2K')}
                  className="text-[10px] font-bold text-blue-700 dark:text-blue-300 bg-blue-100 dark:bg-blue-900/50 px-2 py-0.5 rounded cursor-pointer"
                >
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  { ratio: '16:9', label: '16:9 Web/YT' },
                  { ratio: '4:3', label: '4:3 Catálogo' }
                ].map(item => (
                  <button
                    key={item.ratio}
                    type="button"
                    ...
                    className={`py-2 px-1 text-xs font-bold rounded-lg border transition-all text-center cursor-pointer ${
                      imageRatio === item.ratio 
                        ? 'bg-blue-50 border-blue-600 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400 shadow-xs' 
                  ...
                  >
                    <div>{item.ratio}</div>
                    <div className="text-[9px] font-normal opacity-80 truncate">{item.label.split(' ')[1]}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 shadow-md disabled:opacity-50 transition-all cursor-pointer"
            >
              {isGeneratingImg ? (
                <>
                  <Loader2 size={16} className="animate-spin" />
                  <span>Sintetizando píxeles con Google Gemini Imagen...</span>
                </>
              ) : (
                <>
                  <Sparkles size={16} />
                  <span>Renderizar Imagen Publicitaria</span>
                </>
              )}
            </button>
          ...
          {/* Video Instructions & Prompts */}
          <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-gray-200 dark:border-slate-800 shadow-sm space-y-4">
            <h3 className="font-bold flex items-center gap-2 text-purple-600 dark:text-purple-400 text-sm">
              <Video size={18} /> Instrucciones de Video para la Campaña (Veo / Sora / Runway)
            </h3>
            <p className="text-xs text-slate-500">
              Guiones y prompts estructurados con gancho (Hook), desarrollo y CTA listos para producir:
            </p>
            
            <div className="space-y-3">
              <div className="space-y-1">
                <label className="text-xs font-bold flex items-center gap-1 text-slate-700 dark:text-slate-300">
                  <Instagram size={12} className="text-pink-500"/> TikTok & Reels (Formato Vertical 9:16)
                </label>
                <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 font-mono text-xs text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">
                  {masterBrief?.tiktok?.sceneScript || masterBrief?.tiktokReels?.sceneScript || 'Gancho de 0-3 segundos con paneo rápido, seguido de beneficios clave del producto.'}
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-bold flex items-center gap-1 text-slate-700 dark:text-slate-300">
                  <Youtube size={12} className="text-red-500"/> YouTube & Ads (Formato Horizontal 16:9)
                </label>
                <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 font-mono text-xs text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">
                  {masterBrief?.youtube?.videoIdea || 'Toma panorámica cinemática con iluminación de showroom y rack focus al producto principal.'}
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Right column: Current Image Render & Saved Gallery */}
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-4 border border-gray-200 dark:border-slate-800 min-h-[400px] flex flex-col items-center justify-center">
            {generatedImg ? (
              <div className="w-full space-y-4 animate-in fade-in zoom-in duration-300">
                <div className="relative group rounded-xl overflow-hidden shadow-md border border-gray-200 dark:border-slate-800 bg-black/5 dark:bg-black/40">
                  <img src={generatedImg} alt="Generada" className="w-full object-contain max-h-[460px] mx-auto rounded-lg" />
                </div>
                {appliedPrompt && (
                  <div className="text-[10px] text-slate-500 font-mono p-3 bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 shadow-xs leading-relaxed space-y-1.5">
                    <div className="flex items-center justify-between">
                      <strong className="text-slate-700 dark:text-slate-300">Prompt Traducido y Refinado (IA):</strong>
                      <span className="text-[9px] bg-green-100 dark:bg-green-950/60 text-green-700 dark:text-green-300 font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                        <CheckCircle2 size={10} /> Prompt Refiner
                      </span>
                    </div>
                ...
                {appliedArchSpecs && (
                  <div className="p-3 bg-gradient-to-r from-indigo-50/80 to-blue-50/80 dark:from-indigo-950/40 dark:to-slate-800/80 rounded-lg border border-indigo-200 dark:border-indigo-900/50 text-xs space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[10px] text-slate-600 dark:text-slate-300 leading-snug">
                      <div><strong>📷 Óptica:</strong> {appliedArchSpecs.lensFocalLength}</div>
                      <div><strong>💡 Iluminación:</strong> {appliedArchSpecs.lighting}</div>
                      <div className="md:col-span-2"><strong>🧱 Materiales:</strong> {appliedArchSpecs.materialTextures}</div>
                    </div>
                  </div>
                )}
              ...
              <div className="text-slate-400 dark:text-slate-600 text-center space-y-3 flex flex-col items-center py-12">
                ...
                <p className="text-[11px] text-slate-400 max-w-xs">
                  Haz clic en "Renderizar Imagen Publicitaria" para generar el asset visual con Google Gemini Imagen.
                </p>
              </div>
            )}
          </div>

          {/* Saved Images Gallery (Never lose images!) */}
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-gray-200 dark:border-slate-800 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-black text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                  <span>Galería de Imágenes Guardadas ({savedHistory.length})</span>
                  <span className="text-[10px] bg-green-100 dark:bg-green-950 text-green-700 dark:text-green-300 px-1.5 py-0.5 rounded font-bold">
                    Guardado Permanente
                  </span>
                </h4>
              </div>
              <p className="text-[11px] text-slate-400">
                Tus imágenes permanecen guardadas aunque navegues entre pasos o refresques la página.
              </p>
              <div className="grid grid-cols-3 gap-2.5 max-h-[300px] overflow-y-auto p-1">
                {savedHistory.map((item) => (
                  <div 
                    key={item.id}
                    onClick={() => {
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-1 transition-opacity text-white p-1 text-[10px]">
                      <span>Ver</span>
                      <button
                        type="button"
                        onClick={(e) => {
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-[9px] px-1.5 py-0.5 rounded"
                      >
                        Referencia
                      </button>
                    </div>
      ...
      <div className="pt-6 flex items-center justify-between border-t border-gray-100 dark:border-slate-800">
        <div className="flex items-center gap-3">
          {onBack && (
            ...
            >
              <ArrowLeft size={14} /> Volver a Copies (Paso 2)
            </button>
          )}
        </div>
        <button 
          type="button"
          onClick={onNext}
          className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-3 rounded-xl font-bold text-sm shadow-md hover:shadow-lg transition-all flex items-center gap-2 cursor-pointer"
        >
          Ir a Estrategia y Despliegue <ArrowRight size={16} />
        </button>
      </div>
